...
public class TomaController {
    ...
    public ResponseEntity<Toma> crearToma(@PathVariable Long pacienteId, @RequestParam Long medicacionId, @RequestBody Toma toma) {
        Toma creada = tomaService.crearToma(pacienteId, medicacionId, toma);
        return ResponseEntity.ok(creada);
    }

    @GetMapping("/pacientes/{pacienteId}/tomas")
    public ResponseEntity<List<Toma>> listarTomasPorFecha(@PathVariable Long pacienteId, @RequestParam String fecha) {
    ...
    public ResponseEntity<Toma> actualizarToma(@PathVariable Long tomaId, @RequestBody Toma datos) {
